...
export default {
  ...
  data() {
    return {
      messs: "",   //输出框查询
      items: [{id: "", index: '', product: "",version : '', version_count: "", version_count: "" }],// 列表内容
      page: "",    //总页数
      rowNum: "",  //总数
      pageIndex: 1 //当前页码
    };
  ...
  methods: {
    ...
    dataInit() {
      ...
      .catch(err => {
        this.$store.commit("setLoading", false);
        this.buttonMessage('列表获取失败','error',1500);
      });
    },
    // 搜索
    search() {
    ...
    chon() {
      this.messs = "";
      axioIn.get("/api/product/product/").then(res => {
        this.items = res.data.results;
        this.page = res.data.page_num.total_page;
        this.rowNum = res.data.page_num.total_count;
        this.pageIndex = 1;
    ...
    add() {
      ...
      .then(({ value }) => {
        if(value) {
          axioIn.post('/api/product/product/', {product:value })
          .then(res=> {
    ...
    // 编辑产品
    bian(id) {
      Router.push({ path: "/product", query: { id: id  , type: 0} });
    }, 
    // 分页数据
    incrementTotal(q) {
      let _pageIndex = q === undefined ? 1 : q;
      let API_SEARCH = "/api/product/product/?product_name=" + this.messs + "&page=" + _pageIndex;
      let API_NOSEARCH = "/api/product/product/?page=" + _pageIndex;
