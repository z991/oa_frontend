/*
 * @Author: liuqian 
 * @Date: 2018-01-08 16:17:23 
 * @Last Modified by: liuqian
 * @Last Modified time: 2018-04-23 15:22:24
 */
<template>
    <div class="addService">
        <div class="serviceTitle">产品管理 > 运维配置>重构版服务>{{b3}}</div>
        <div class='revise'>
            <div class="inContent">
            ...
            <div v-for='(item,index) in b4.ser_url' :key="index">
                <p class='serAdd'><label for="">服务地址</label>
                    ...
                    <input type="text" v-model='item.ser_address' style='width:240px;'><em class='ii'>*</em><a href="javascript:void(0)" class='addAddress' @click='add_address()'></a><a href="javascript:void(0)" class='quu' @click='dele_address(index)'></a></p>
                <p v-for='(a,index) in item.ser_ip' :key="index" class='serAdd'>
                    <label for="" >IP</label><!--style="margin-left:20px;"-->
                <input type="text" v-model='a.ser_ip' style='width:240px;'><em class='ii' >*</em><a href="javascript:void(0)" class='addIp' @click='add_ip(item.ser_ip)'></a><a href="javascript:void(0)" class='quu' @click='dele_ip(item.ser_ip,index)'></a></p>
            </div>
            ...
            <p class='bao' style='margin:0;text-align:right;height:72px;'><a href="javascript:void(0)" class='quu' @click='close()'>取消</a><a href="javascript:void(0)" class='qub' @click='baob(b4.id)'>保存</a></p>
        </div>
...
export default {
  ...
  mounted: function(){
      ...
      if(this.$route.query.id===undefined){
          ...
          this.b4={ser_id:'',version_type: 2,ser_name:'',ser_url:[{ser_address:'',ser_ip:[{ser_ip:''}]}]};
      }else{
          this.id = this.$route.query.id
          this.adisable=true;this.b3 = '修改'
          axioIn.get('/api/product/server/'+this.$route.query.id+'/?version_type=2').then(
              (res)=>{
                  this.adisable=true;
                  this.b4=res.data;
  ...
  methods: {
      close:function(){
          if(this.b3 === '修改'){
              Router.push({ path: "/refactorServe",query: {page: this.page,search: this.searchTerm}});
          }else if(this.b3 === '新增'){
              Router.push({ path: "/refactorServe"});return
          }
       },
          ...
          if(bb===true){
            ...
            if(this.b3==='新增'){
                ...
                axioIn.post('/api/product/server/',this.b4).then(
                    ...
                    //   this.shad=false; this.b1=false;
                      alert('新增成功')
                      Router.push({ path: "/refactorServe"});
...
@import '../assets/home';
.addService{width:90%;margin:auto;background:#edf2f5;
    // .serviceTitle{width:100%;height:40px;line-height: 40px;background:#fff;margin:15px 0;}
    // .revise{width:100%px;background-color:white;
    // .inContent{height: 656px;width:100%;border-bottom:1px solid #ccc;overflow-y: scroll;}
    // .bao{
    //     .qub{margin:20px 40px 0 10px;}
    // }
    // .serAdd{text-align:left;
