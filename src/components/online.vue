<template>
    <div class="lineStation">
      ...
      <p class='tipp'><span :class='{sactiv:span_act}' @click='change_tab()'>行业站点数量及占比</span><span  @click='change_tab()' :class='{sactiv:!span_act}'>部署方式站点数量及占比</span><a href="javascript:void()" class='out' @click='expor()'>导出</a></p>
      ...
      <div class='chart' v-show='!span_act'>
            <p class="ji">
                ...
                <span class='jii'><label for="">日期</label>
                <input type="text" v-model="gues.date" @focus='focu1()' readonly="true" ></span>
                <datee v-if='date_s1' v-on:incrementt="incrementTi1"  @mouseleave.native='hide()'  class='dat'></datee>
      ...
      <div style='background-color:white;'>
          <p class='tipp'><span  >站点数量变化趋势</span><a href="javascript:void()" class='out' @click='export1()'>导出</a></p>
          <p class='lef mar20'>
...
export default {
    ...
    mounted:function(){
        ...
        //  axioIn.get('/api/data/online-client/online_consumer_allday/?idpy=1|2&start_date='+bb+'&end_date='+bb)
        //     .then((res)=>{
   ...
   methods:{
        ...
        search_char:function(){
            ...
            .then((res)=>{
                res.data.data.map(n=>{n.type='line';n.showAllSymbol=true});
                this.trade2.series=res.data.data;
                this.trade2.legend.data=res.data.data.map(n=>n.name)
                //console.log(JSON.stringify(res.data.data))
                //console.log(JSON.stringify(this.trade2.series))
...
<style lang='scss' scoped>
    .lineStation{
        ...
        .chart{width:100%;height:400px;background-color: white;position:relative}
