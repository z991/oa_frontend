<template>
  <div class="chanpf">
      ...
      <div v-show='b1' class='revise'>
          ...
          <div v-for='(item,index) in b4.ser_url'>
                ...
                <p v-for='(a,index) in item.ser_ip' class='bao'>
          ...
          <p class='bao' style='margin:0;'><a href="javascript:void(0)" class='quu' @click='close()'>取消</a><a href="javascript:void(0)" class='qub' @click='baob(b4.id)'>保存</a></p>
      </div>
      <shade v-show='shad'></shade>
      <shade v-show='shad1' style='z-index:16'></shade>
      <pagination :number='page' v-on:increment="incrementTotal" :pageIndex='pagI'></pagination>
...
export default {
  data:function(){
      ...
      b3:'',
      items:[],
      b4:{ser_id:'',ser_name:'',ser_urll:'',ser_url_list:'',ser_url:[{ser_address:'',ser_ip:[{ser_ip:''}]}]},
      b5:'',
      adisable:false,
      b6:'',
      page:'',//总数据条数
  ...
  components:{Shade,Pagination},
   created:function(){
   this.$checkCookie();
   axioIn.get('/api/product/sertype/forserver/').then((res)=>
       this.items=res.data
   )   
  },
  mounted:function(){
      axioIn.get('/api/product/server/')
      .then((res) => {
  ...
  methods:{
      cha:function(id){
          ...
          if(this.b3==='修改'){
          axioIn.get('/api/product/server/'+id+'/').then(
              (res)=>{
                  this.b4=res.data;
                  let aii=[];
      ...
      search:function(){
          if(this.messs!==''){
          axioIn.get('/api/product/server/?ser_id='+this.messs)
      ...
      add:function(){
         ...
         this.b1=true;this.b4={ser_id:'',ser_name:'',ser_urll:'',ser_url_list:'',ser_url:[{ser_address:'',ser_ip:[{ser_ip:''}]}]};
       },
      incrementTotal:function(q){
          q=(q===undefined?1:q);
          if(this.search_open===false){//console.log(q===undefined)
              
              axioIn.get('/api/product/server/?page='+q)
              .then((res) => {
                this.popp=this.b2=res.data.results;
                this.page=res.data.page_num.total_page;
                this.rowNum = res.data.page_num.total_count;
              ...
              axioIn.get('/api/product/server/?ser_id='+this.messs+'&page='+q)
              .then((res) => {
                this.popp=this.b2=res.data.results;
                this.page=res.data.page_num.total_page;
                this.rowNum = res.data.page_num.total_count;
      ...
      delet:function(id,index){
         confirm('确认删除？')?
          axioIn.delete('/api/product/server/'+id+'/')
            ...
            if(this.b3==='修改'){//console.log(undefined==0)
                axioIn.put('/api/product/server/'+id+'/',this.b4).then(
                    ...
                    .then((res) => {
                        ...
                        axioIn.get('/api/product/server/?page='+this.pagI)
                        .then((res) => {
                            this.popp=this.b2=res.data.results;
                            this.page=res.data.page_num.total_page;
                            this.rowNum = res.data.page_num.total_count;
                        ...
                        axioIn.get('/api/product/server/?ser_id='+this.messs+'&page=1')
                        .then((res) => {
                            this.popp=this.b2=res.data.results;
                            this.page=res.data.page_num.total_page;
                            this.rowNum = res.data.page_num.total_count;
    ...
    dele_address:function(index){console.log(index);this.b4.ser_url.splice(index,1)},
    dele_ip:function(pi,i){pi.splice(i,1);console.log(i)},
  }
}
</script>
