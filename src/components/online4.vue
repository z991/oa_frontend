<template>
    <div class="line4">
        <p class="qu">数据管理 > 产品与运营统计 >站点运营情况</p>
      <p class='tipp'><span>站点运营情况及占比</span><a href="javascript:void()" class='out' @click='expor()'>导出</a></p>
      <div class='chart' v-show='span_act'>
            <p class="ppi">
                <span class=''><label for="">行业</label>
                <select name="" id="" v-model='deplo.industry'>
                    <option value='0' >所有</option>
                    <option :value="it.id" v-for='(it,index) in biao' :key="index">{{it.industry}}</option>
                </select></span>
                <span class=''><label for="">部署方式</label>
                ...
                <span ><a href="javascript:void(0)" class="out" @click='search_bu()'>确定</a></span>
                <!--span class='jii'><span>客户数：{{pp}}</span></span-->
                
            </p>
            <!--p class="xinz"><a href="javascript:void(0)"  class="so" @click='add()'>新增</a></p-->
            <div id="myChart" :style="{width: '900px', height: '400px',float:'left'}"></div>
      </div>
      <div style='background-color:white;'>
          <p class='tipp'><span>站点运营变化趋势</span><a href="javascript:void()" class='out' @click='export1()'>导出</a></p>
          <p class='lef mar20'>
          ...
          <div class='lef mar20'>
              ...
              <div style='width:500px;position:absolute;border:1px solid black;z-index:40000;background:white;' v-show='indushow' @mouseleave='lose_focu()'>
                  ...
                  <p  class='left'> 站点运营类型<span style='color:#0386f0'>（必选）</span></p>
                  <ul  class='indus'>
                      <li v-for='(ii,index) in deployi' :class='{dactiv:ii.a1}' @click='choose2(index)' :id='ii.id' :key="index">{{ii.name}}</li>
                  </ul>
...
export default {
    ...
    mounted:function(){
        ...
        axioIn.get('/api/data/site-oper/get_site_oper/?start_date='+bb+'&end_date='+bb+'&industry='+this.deplo.industry+'&deploy_way='+this.deplo.deploy_way)
        .then((res)=>{
            this.trade.series[0].data=res.data.data;
            this.trade.xAxis[0].data=res.data.name;
            this.drawLine();
        })
        ...
        axioIn.get('/api/workorder/customer-oper/')
         .then((res)=>{
            this.deployi=res.data.data;
            this.deployi.map(n=>this.$set(n,'a1',false));
            })
        /*axioIn.get('/api/data/site-oper/get_site_oper_allday/?idpyop=1|2|8&start_date='+bb+'&end_date='+bb)
         .then((res)=>{
             if(res.data.data.length!==0){
   ...
   methods:{
        ...
        add_char:function(){
            ...
            if(aaq.length===0){alert('站点运营类型为必选');return}
        ...
        search_bu:function(){
            ...
            if(aa===true){alert('请完善输入信息')}
                ...
                axioIn.get('/api/data/site-oper/get_site_oper/?start_date='+this.deplo.start_date+'&end_date='+this.deplo.end_date+'&industry='+this.deplo.industry+'&deploy_way='+this.deplo.deploy_way)
                .then((res)=>{
                        this.trade.series[0].data=res.data.data;
                        this.trade.xAxis[0].data=res.data.name;
                        this.drawLine();
        ...
        search_char:function(){
            ...
            if(aad<bbd){alert('开始时间和结束时间不能超过2个月')}
                ...
                let aa=this.itee.join('&idpyop=');
                //console.log(aa)
                axioIn.get('/api/data/site-oper/get_site_oper_allday/?idpyop='+aa+'&start_date='+this.ke.start_date+'&end_date='+this.ke.end_date)
            .then((res)=>{
                if(res.data.data.length!==0){
                res.data.data.map(n=>n.type='line');
                this.trade2.series=res.data.data;
        ...
        expor:function(){
            //if(this.span_act===true){
                if(this.deplo.start_date===''||this.deplo.end_date===''){alert('日期不可为空')}
                  ...
                  window.open('/api/data/site-oper/csv_site_oper/?start_date='+this.deplo.start_date+'&end_date='+this.deplo.end_date+'&industry='+this.deplo.industry+'&deploy_way='+this.deplo.deploy_way)
                }
            //}
           /* else{
        ...
        export1:function(){
            ...
            window.open('/api/data/site-oper/csv_site_oper_allday/?&idpyop='+aa+'&start_date='+this.ke.start_date+'&end_date='+this.ke.end_date)
        }}
   }
...
<style lang='scss' scoped>
    .line4{
        ...
        .ppi{text-align:left;padding-top:10px;position: relative;
            label{display: inline-block;width:70px;text-align:right;padding-right:10px;}
            select,input{border-radius:5px;border:1px solid gray;}
            select{width:202px;height:34px;margin-bottom:10px;option{padding:4px;}}
            input{width:200px;height:32px;margin:0;padding:0 5px;}
        }
        .so{margin:30px 94px;}
        ...
        .chart{width:100%;height:500px;background-color: white;position: relative;}
