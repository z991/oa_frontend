<template>
    <div class="line3">
        <p class="qu">数据管理 > 产品与运营统计 >线上产品情况</p>
      <p class='tipp'><span >单日线上产品数量及占比</span><a href="javascript:void()" class='out' @click='expor()'>导出</a></p>
      <div class='chart'>
            <p class="ji">
      ...
      <div style='background-color:white;'>
          <p class='tipp'><span  >线上产品数量变化趋势</span><a href="javascript:void()" class='out' @click='export1()'>导出</a></p>
          <p class='lef mar20'>
          ...
          <div class='lef mar20'>
              ...
              <div style='width:500px;position:absolute;border:1px solid black;z-index:40000;background:white;' v-show='indushow' @mouseleave='lose_focu()'>
                  ...
                  <ul  class='indus'>
                      <li v-for='(ii,index) in deployi' :class='{dactiv:ii.a1}' @click='choose2(index)' :id='ii.id' :key="index">{{ii.product}}</li>
                  </ul>
...
export default {
    ...
    data:function(){
        return{
            ...
            item:[],
            itee:[],
            deployy:[{value:'公有云',a1:false,id:'2'},{value:'专属云',a1:false,id:'3'},{value:'标准版',a1:false,id:'1'},{value:'私有云',a1:false,id:'4'}],//部署方式
            industry:[],//行业
            indushow:false,
            start_date:'',
            end_date:'',
    ...
    mounted:function(){
        ...
        axioIn.get('/api/data/online-product/get_day_product/?date='+this.deplo.date+'&industry='+this.deplo.industry+'&deploy_way='+this.deplo.deploy_way)
        .then((res)=>{
        ...
        axioIn.get('/api/product/product/fordata/')
        .then((res)=>{
            this.deployi=res.data;
   ...
   methods:{
        ...
        //     if(!reg.test(this.deplo.date))this.deplo.date = ''
        // },
        add:function(){
        ...
        add_char:function(){
            ...
            if(this.item.length>9){alert('最多添加10个指数');return}
                ...
                this.industry.map(n=>{if(n.a1===true){aa=n.industry;dd=n.id}})////行业
                this.deployy.map(n=>{if(n.a1===true){bb=n.value;cc=n.id}})//部署方式
                this.deployi.map(n=>{if(n.a1===true){ee=n.product;ff=n.id}})//产品
        ...
        search_bu:function(){
            if(this.deplo.deploy_way===''||this.deplo.date===''||this.deplo.industry===''){alert('请完善输入信息')}
            else{
                axioIn.get('/api/data/online-product/get_day_product/?date='+this.deplo.date+'&industry='+this.deplo.industry+'&deploy_way='+this.deplo.deploy_way)
                .then((res)=>{
                    let aa= res.data.data.filter(n=>n.value>0)
        ...
        search_char:function(){
            ...
            if(aad<bbd){alert('开始时间和结束时间不能超过2个月')}
                ...
                let aa=this.itee.join('&idpypd=');
                console.log(aa)
                axioIn.get('/api/data/online-product/get_product_allday/?idpypd='+aa+'&start_date='+this.ke.start_date+'&end_date='+this.ke.end_date)
            .then((res)=>{
                console.log(res)
                if(res.data.data.length!==0){
                res.data.data.map(n=>n.type='line');
                ...
                console.log(this.trade2.series)
        ...
        export1:function(){
            ...
            window.open('/api/data/online-product/csv_product_allday/?idpypd='+aa+'&start_date='+this.ke.start_date+'&end_date='+this.ke.end_date)
        }},
   }
